...
class ReportPdf:
    ...
    def __init__(self, list,file_name):
        ...
        list=list
        archivo = Archivo('Datos de los clientes', datetime.now().strftime('%d/%m/%Y'))

        
        document_title = 'Clientes de la Empresa'
        ...
        canvas.drawCentredString(385+100, 785-250, nombre_colaborador)
        canvas.setFont("Helvetica", 14)
        canvas.drawCentredString(385+100, 765-250, fecha_actual)

        title_background = colors.fidblue
        data_actividades = [
            ['Id', 'Nombre', 'Tipo de cliente', 'Direccion',"Telefono"],
        ]
        for i in list:
            data_actividades.append([i[0], i[1], i[2], i[3], i[4]])
            table_actividades = Table(data_actividades, colWidths=150, rowHeights=15, repeatRows=0)
            tblStyle = TableStyle([
